...
fun main() {
    val codes = File("/Users/jwhiting/IdeaProjects/aoc2018/src/Day2.txt").bufferedReader().readLines()

    val twoCount = codes.map { counts(it) }.filter {
        hasCount(
            it,
            2
        )
    }.count()
    val threeCount = codes.map { counts(it) }.filter {
        hasCount(
            it,
            3
        )
